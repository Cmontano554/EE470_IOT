...
<body>
    ...
    $conn->close();
    ?>
<!-- ====================== TEMPERATURE & HUMIDITY LINE CHART WITH NODE SELECT ====================== -->
<div style="display: flex; justify-content: center; margin-top: 40px;">
  <div style="
      ...
      <h3 style="margin-bottom: 15px; color: #2e7d32;">Temperature & Humidity vs Time</h3>

      <!-- Dropdown Menu for Node Selection -->
      <label for="nodeSelect" style="font-weight: bold; color: #333;">Select Node:</label>
      <select id="nodeSelect" style="margin: 10px; padding: 5px;">
        <?php
        include 'db_connect.php';
        $nodes = $conn->query("SELECT DISTINCT node_name FROM sensor_data ORDER BY node_name");
        if ($nodes && $nodes->num_rows > 0) {
            while ($n = $nodes->fetch_assoc()) {
                echo "<option value='{$n['node_name']}'>{$n['node_name']}</option>";
            }
        }
        $conn->close();
        ?>
      ...
      <canvas id="tempChart" width="500" height="300"></canvas>
  </div>
</div>
        ...
        datasets: [
            {
                ...
                data: [],
                fill: false,
                borderColor: 'rgba(56, 142, 60, 1)',   // green
    ...
    options: {
        scales: {
            x: {
                ...
                ticks: { color: '#000', autoSkip: true, maxTicksLimit: 6 },
        ...
        plugins: {
            legend: { position: 'top', labels: { color: '#000', boxWidth: 20, boxHeight: 10 } }
        }
    }
});

// Function to load data dynamically for selected node
async function loadNodeData(nodeName) {
    const response = await fetch(`data_json_dynamic.php?node=${encodeURIComponent(nodeName)}`);
    const data = await response.json();

    const times = data.map(d => d.time_received);
...
// Load initial node on page load
const nodeSelect = document.getElementById('nodeSelect');
loadNodeData(nodeSelect.value);

// Update chart when selection changes
nodeSelect.addEventListener('change', function() {
    loadNodeData(this.value);
});
...
echo "<h2>Sensor Activity</h2>";

include 'db_connect.php'; // reconnect to DB (since it was closed earlier)

$sql = "SELECT node_name AS 'Node Name', count AS 'Message Count' 
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr>
                <td>{$row['Node Name']}</td>
                <td>{$row['Message Count']}</td>
              </tr>";
    }
    echo "</table>";
} else {
    echo "<p>No data in sensor_activity table.</p>";
}
